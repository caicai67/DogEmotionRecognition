...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):
        ...
        """
        """YOUR CODE"""
        weightedActs = list(inActs)
        weightedActs.insert(0, 1)
        weighted = self.getWeightedSum(weightedActs)
        activation = self.sigmoid(weighted)
        return activation
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        weightedActs.insert(0, 1)
        weighted = self.getWeightedSum(weightedActs)
        activation = self.sigmoidDeriv(weighted)
        return activation
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        totalModification = 0
        """YOUR CODE"""
        biasedActs = list(inActs)
        biasedActs.insert(0, 1)
        for i in range(0, len(self.weights)):
            oldWeight = self.weights[i]
            newWeight = oldWeight + (alpha*delta*biasedActs[i])
            totalModification += abs(newWeight - oldWeight)
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        out = list()
        currentActs = inActs
        out.append(currentActs)
        for layer in self.layers:
            newActs = list()
            for perceptron in layer:
                newActs.append(perceptron.sigmoidActivation(currentActs))
            out.append(newActs)
            currentActs = newActs
        return out
        ...
        for example in examples:#for each example
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    gPrime = layer[neuronNum].sigmoidActivationDeriv(example[0]) #"""FILL IN""")
                    delta = gPrime*sum((nextLayer[i].weights[neuronNum+1] * outDelta[i]) for i in range(len(nextLayer)))#"""FILL IN - delta for this neuron
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    iteration=0
    trainError=0
    #value doesn't matter so arbitrarily making weightMod twice the threshold
    weightMod=weightChangeThreshold*2
    while iteration < maxItr and weightMod > weightChangeThreshold:
        iteration+=1
        error, weightMod = nnet.backPropLearning(examples[0], alpha)
        trainError += error
    if iteration > 0:
        trainError /= iteration
    ...
    for example in examples[1]:
        outputLayer = nnet.feedForward(example[0])[-1]
        correct = True
        for i in range(len(outputLayer)):
            if not (round(outputLayer[i]) == example[1][i]):
                correct = False
        if correct:
            testCorrect+=1
        else:
